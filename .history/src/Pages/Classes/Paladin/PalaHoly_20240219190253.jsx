import React from 'react'

export default function PalaHoly() {
   return (
    <div className="w-full flex flex-col justify-center items-center gap-2  mb-10">
      <h2 className="text-2xl">Estadísticas para raids de 10J</h2>
      <span className="bg-gradient-to-t from-[#c7d2fe] to-[#8678f9] bg-clip-text text-xl text-transparent">
        {`Intelecto > Espíritu > Celeridad (hasta conseguir el 25.03%) > Maestría > Maestría`}
      </span>


    </div>
  );
}




Intellect;
Spirit;
Haste Rating (until the 25.03% soft cap, seebelow);
Mastery Rating;
Haste Rating (until the 34.98% soft cap);
Critical Strike Rating.
